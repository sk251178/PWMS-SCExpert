...
function Page_ClientValidate() {
	...
	if (enableAlerts) {
		if (errorValIndex != -1) {
			var val = Page_Validators[errorValIndex];
			var allowAlert = true;
			if (''+val.AlertStartText == 'undefined' && ''+val.AlertEndText == 'undefined' && ''+val.FieldLabel == 'undefined') {
				allowAlert = false;
			}
			if (allowAlert) {
				var alertText = val.AlertStartText + '\n\n';
				if (''+val.FieldLabel != 'undefined') alertText += val.FieldLabel + '\n\n';
				alertText += val.AlertEndText;
				alert(alertText);
			}
		}
	}
...
function UpdateValidatorAttributes(val) {    
...
function ValidatorOnLoad() {
    ...
    for (i = 0; i < Page_Validators.length; i++) {
        ...
        if (m4nBrowserName=='Explorer' && m4nBrowserVersion>8) UpdateValidatorAttributes(val);
...
function CompareValidatorEvaluateIsValid(val) {
    ...
    var result = ValidatorCompare(value, compareTo, val.operator, val);
...
function CustomValidatorEvaluateIsValid(val) {
	...
	////////////////////////////////////
	// added by Made4Net    
    if (!args.IsValid) errorFound = true;
	/////////////////////////////////////
...
function RegularExpressionValidatorEvaluateIsValid(val) {
	...
	////////////////////////////////////
	// Modified by Made4Net    
    var result = (matches != null && value == matches[0]);
    if (!result) {
		errorFound = true;
	}
	return result;
...
function RangeValidatorEvaluateIsValid(val) {
	...
	////////////////////////////////////
	// Modified by Made4Net    
    var result = (ValidatorCompare(value, val.minimumvalue, "GreaterThanEqual", val) && ValidatorCompare(value, val.maximumvalue, "LessThanEqual", val));
    if (!result) errorFound = true;
	return result;
	///// original version:
