...
function goMouseDown(e, id) {
  ...
  if (e.srcElement && e.srcElement.setCapture) {
    goCp = e.srcElement;
    if (goCp.style.cursor == 'auto') goCp.style.cursor = 'move';
    goCp.setCapture();
  }
}
...
function goShowMenu(menuid) {
  ...
  goMenuID = menuid;
  var r = document.body.offsetWidth-20-m.offsetWidth;
  if (x >= r) x = r;
  var b = document.body.offsetHeight-20-m.offsetHeight;
  if (y >= b) y = b;
  m.style.left = x;
  m.style.top = y;
...
if (goIE) {
  ...
  document.captureEvents(Event.MOUSEDRAG | Event.MOUSEMOVE);
  document.onmousemove = goNSDocMove;
