...
#Region "Constructors"
        ...
        Public Sub New(ByVal pTemplateName As String, Optional ByVal pLogger As LogHandler = Nothing)
...
#Region "Assign To Shipment"
        ...
        Private Sub BuildShipment(ByVal pOrdersAL As ArrayList)
            ...
            For i As Int32 = 0 To pOrdersAL.Count - 1
                ...
                pOrdersAL(i)("QTYOPEN"), 0, WMS.Lib.USERS.SYSTEMUSER)

                If i = pOrdersAL.Count - 1 Then
                    oShipment.UpdateShipmentStatus()
                ElseIf i + 1 Mod mMAxNumberOfOrders = 0 Then
                    oShipment.UpdateShipmentStatus()
                End If

            Next
            oShipment.CalculateStdtime()
        End Sub

        Public Sub AssignOrder(ByVal oShip As WMS.Logic.Shipment, ByVal Consignee As String, ByVal OrdId As String, ByVal pOrderLine As Int32, ByVal pUnits As Decimal, ByVal pLoadingSequence As Int32, ByVal pUser As String, Optional ByVal pDocumentType As String = WMS.Lib.DOCUMENTTYPES.OUTBOUNDORDER)
            If oShip.STATUS = WMS.Lib.Statuses.Shipment.SHIPPED Or oShip.STATUS = WMS.Lib.Statuses.Shipment.CANCELED Then
            ...
            Select Case pDocumentType
                Case WMS.Lib.DOCUMENTTYPES.OUTBOUNDORDER
                    Dim oShipDetail As New WMS.Logic.Shipment.ShipmentDetail(oShip.SHIPMENT, Consignee, OrdId, pOrderLine)
                    'oShipDetail = WMS.Logic.Shipment.ShipmentDetails.getShipmentDetail(Consignee, OrdId, pOrderLine)
                    ...
                    Else
                        ...
                        '    oShipDetail.MoveToShipment(oShip.shipment, pUser)
                        '    oShipDetail.AddQauntity(pUnits, pUser)
                        'Else
                        oShipDetail = New WMS.Logic.Shipment.ShipmentDetail
                        oShipDetail.Create(oShip.SHIPMENT, Consignee, OrdId, pOrderLine, pUnits, pLoadingSequence, pUser, pDocumentType)
                        oShip.ShipmentDetails.add(oShipDetail)
                        'End If
                    End If
                Case WMS.Lib.DOCUMENTTYPES.FLOWTHROUGH
                    Dim fl As New Flowthrough(Consignee, OrdId)
                    fl.AssignToShipment(oShip.SHIPMENT, pUser)
                    ' Flowthroughs.add(fl)
                Case WMS.Lib.DOCUMENTTYPES.TRANSHIPMENT
                    Dim t1 As New TransShipment(Consignee, OrdId)
                    t1.AssignToShipment(oShip.SHIPMENT, pUser)
                    ' Transshipments.add(t1)
            End Select
            ' CalculateStdtime()
            oShip.EDITDATE = DateTime.Now
            oShip.EDITUSER = pUser
            If oShip.STATUS = WMS.Lib.Statuses.Shipment.STATUSNEW Then
                oShip.STATUS = WMS.Lib.Statuses.Wave.ASSIGNED
                DataInterface.RunSQL(String.Format("Update shipment set STATUS = {0}, EDITDATE = {1},EDITUSER = {2} WHERE {3}", Made4Net.Shared.Util.FormatField(oShip.STATUS), Made4Net.Shared.Util.FormatField(oShip.EDITDATE), Made4Net.Shared.Util.FormatField(oShip.EDITUSER), WhereClause(oShip.SHIPMENT)))
            Else
                DataInterface.RunSQL(String.Format("Update shipment set EDITDATE = {0},EDITUSER = {1} WHERE {2}", Made4Net.Shared.Util.FormatField(oShip.EDITDATE), Made4Net.Shared.Util.FormatField(oShip.EDITUSER), WhereClause(oShip.SHIPMENT)))
            End If
        End Sub

        Public ReadOnly Property WhereClause(ByVal Shipment As String) As String
            Get
