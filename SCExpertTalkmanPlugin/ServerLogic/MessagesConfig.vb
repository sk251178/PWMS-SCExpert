...
Public Class MessagesInfo
    ...
    'Private Shared _receivedMessage As String
    Private Shared _config As MessagesInfo
    Private Shared mut As New Mutex()

    Private Sub Init(ByVal oLogger As WMS.Logic.LogHandler)
        ...
        Catch ex As Exception
            If Not oLogger Is Nothing Then
                oLogger.Write(String.Format("MessagesConfig.Init() Error has occured when initiating message config . Error details:"))
    ...
    Public Shared Sub CreateConfig()
        ...
        '_config.Init()
        MessagesInfo.CreateConfig(Nothing)
    End Sub
    Public Shared Sub CreateConfig(ByVal oLogger As WMS.Logic.LogHandler)
        If Not oLogger Is Nothing Then
            oLogger.Write("MessagesConfig.CreateConfig() Entering protected state")
        End If
        mut.WaitOne()
        If Not oLogger Is Nothing Then
            oLogger.Write("MessagesConfig.CreateConfig() Protected state.")
        End If
        _config = New MessagesInfo
        _config.Init(oLogger)
        If Not oLogger Is Nothing Then
            oLogger.Write("MessagesConfig.CreateConfig() Protected state code finished.")
        End If
        mut.ReleaseMutex()
        If Not oLogger Is Nothing Then
            oLogger.Write("MessagesConfig.CreateConfig() Protected state exit.")
        End If
    End Sub
    ...
    Public Shared ReadOnly Property EOF() As String
        ...
        End Get
    End Property
    Public Shared Function GetMessageConfig(ByVal MessageName As String, ByVal oLogger As WMS.Logic.LogHandler) As MessageInfo
        'Return _config._messages(MessageName)
        Dim msgInfo As MessageInfo
        SyncLock _config._messages.SyncRoot
            msgInfo = _config._messages(MessageName)
        End SyncLock
        Return msgInfo
    End Function
    ...
    Public Shared Function GetMessageInstance(ByVal MessageData As String) As ClientMessage
        ...
        Dim clmsg As ClientMessage = Nothing
    ...
    Public Class MessageInfo
        ...
        Private Sub Parse(ByVal MessageDR As DataRow)
            ...
            Made4Net.DataAccess.DataInterface.FillDataset("select * from ScExpertTalkManMessageFields where MessageName='" & Me.Name & "' Order by ordinal", MsgFieldsDT, False, "Made4NetSchema")
            ...
            If Not _dynfieldsview.Equals("") Then
                ...
                Made4Net.DataAccess.DataInterface.FillDataset("select * from ScExpertTalkManMessageDynamicFields where MessageName='" & Me.Name & "' Order by ordinal", MsgDynFieldsDT, False, "Made4NetSchema")
