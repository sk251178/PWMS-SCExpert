...
Public Class InboundAgreementDetail
    ...
    Public Function ProcessTotalBillingValue(ByVal orderid As String, ByVal logger As WMS.Logic.LogHandler, ByVal pUser As String) As Double
        ...
        For Each dr In dt.Rows
            ...
            Select Case _billbasis
                ...
                Case AgreementDetail.BillingBasis.NumberOfDucments
                    ...
                    TotalBillingValue = Me.getChargeValue(dt.Rows.Count, "ORDERID", dr("ORDERID"), 1, logger)
            ...
            TotalBillingValue += Me.getChargeValue(LineBilling, "ORDERID", dr("ORDERID"), 1, logger)
        Next
        ...
        If (TotalBillingValue < _minperrun) Then
            logger.Write("Total Value is Less then Minimum for run: " & _minperrun & " Total Value Updated to Minimum")
    ...
    Public Function ProcessBillingChargeDetailPosted(ByVal BILLFROMDATE As Date, ByVal BILLTODATE As Date, ByVal logger As WMS.Logic.LogHandler, ByVal pUser As String, Optional ByVal ch As ChargeDetail = Nothing) As Int32
        ...
        Try
            ...
            If Not _skugroup Is Nothing And Not _skugroup = "" Then
                sql = String.Format("select rd.consignee, rd.orderid, count(distinct rd.sku) as numskus, count(distinct rd.receiptline) as numlines, " & _
                    "(select count(distinct loadid) from inventorytrans left outer join sku on sku.consignee = inventorytrans.consignee and inventorytrans.sku = sku.sku where invtrntype={0} and inventorytrans.consignee={2} and IsNull(sku.STORAGECLASS,{5}) = {5}  " & _
                    " and document in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID) and isnull(sku.skugroup,'')={3}) - " & _
                    "(select isnull(count(distinct loadid),0) from inventorytrans left outer join sku on sku.consignee = inventorytrans.consignee and inventorytrans.sku = sku.sku where invtrntype={4} and inventorytrans.consignee={2} and IsNull(sku.STORAGECLASS,{5}) = {5} " & _
                    " and document in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID) and isnull(sku.skugroup,'')={3}) as numloads,  " & _
                    "(SELECT SUM(qtyreceived) FROM RECEIPTDETAIL left outer join sku on sku.consignee = RECEIPTDETAIL.consignee and RECEIPTDETAIL.sku = sku.sku WHERE RECEIPT in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID) AND qtyreceived > 0 and isnull(sku.skugroup,'')={3}) as qtyreceived,  " & _
                    "sum(rd.qtyreceived * isnull(sku.unitprice,0)) as value, rhweight.weight as weight,  rhvol.volume as volume  " & _
                    "from receiptheader rh join receiptdetail rd on rh.receipt = rd.receipt join sku on rd.consignee = sku.consignee and rd.sku = sku.sku " & _
                    "join (SELECT rd.CONSIGNEE, rd.ORDERID, sum(ISNULL(SKUUOM.VOLUME, 0) * qtyreceived) AS volume FROM receiptdetail AS rd INNER JOIN " & _
                    "SKUUOM ON rd.CONSIGNEE = SKUUOM.CONSIGNEE AND rd.SKU = SKUUOM.SKU WHERE (SKUUOM.LOWERUOM IS NULL) OR (SKUUOM.LOWERUOM = '') group by rd.CONSIGNEE, rd.ORDERID) AS rhvol ON rhvol.CONSIGNEE = rd.CONSIGNEE and  rhvol.ORDERID = rd.ORDERID " & _
                    "join (SELECT rd.CONSIGNEE, rd.ORDERID, sum(ISNULL(SKUUOM.netweight, 0) * qtyreceived) AS weight FROM receiptdetail AS rd INNER JOIN " & _
                    "SKUUOM ON rd.CONSIGNEE = SKUUOM.CONSIGNEE AND rd.SKU = SKUUOM.SKU WHERE (SKUUOM.LOWERUOM IS NULL) OR (SKUUOM.LOWERUOM = '') group by rd.CONSIGNEE, rd.ORDERID) AS rhweight ON rhweight.CONSIGNEE = rd.CONSIGNEE and  rhweight.ORDERID = rd.ORDERID " & _
                    "left outer join inboundordheader id on id.consignee = rd.consignee and id.orderid = rd.orderid  " & _
                    "where rh.status = {1} and rd.consignee = {2} and rd.qtyreceived > 0 ", _
                    Made4Net.Shared.Util.FormatField(WMS.Lib.Actions.Audit.CREATELOAD), _
                    Made4Net.Shared.Util.FormatField(WMS.Lib.Statuses.Receipt.CLOSED), _
                    Made4Net.Shared.Util.FormatField(_agreement.Consignee), _
                    Made4Net.Shared.Util.FormatField(_skugroup), Made4Net.Shared.Util.FormatField(WMS.Lib.Actions.Audit.UNRECEIVELOAD), _
                    Made4Net.Shared.Util.FormatField(_storageclass))
            Else
                sql = String.Format("select rd.consignee, rd.orderid, count(distinct rd.sku) as numskus, count(distinct rd.receiptline) as numlines, " & _
                    "(select count(distinct loadid) from inventorytrans left outer join sku on sku.consignee = inventorytrans.consignee and inventorytrans.sku = sku.sku where invtrntype={0} and inventorytrans.consignee={2} and  IsNull(sku.STORAGECLASS,{4}) = {4} and document  in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID)) - " & _
                    "(select isnull(count(distinct loadid),0) from inventorytrans left outer join sku on sku.consignee = inventorytrans.consignee and inventorytrans.sku = sku.sku where invtrntype={3} and inventorytrans.consignee={2}  and IsNull(sku.STORAGECLASS,{4}) = {4} and document  in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID)) as numloads,  " & _
                    "(SELECT SUM(qtyreceived) FROM RECEIPTDETAIL left outer join sku on sku.consignee = RECEIPTDETAIL.consignee and RECEIPTDETAIL.sku = sku.sku WHERE RECEIPT  in (select RECEIPT from RECEIPTDETAIL r1 where r1.CONSIGNEE =  rd.CONSIGNEE and r1.ORDERID = rd.ORDERID) AND qtyreceived > 0) as qtyreceived,  " & _
                    "sum(rd.qtyreceived * isnull(sku.unitprice,0)) as value, rhweight.weight as weight,  rhvol.volume as volume  " & _
                    "from receiptheader rh join receiptdetail rd on rh.receipt = rd.receipt join sku on rd.consignee = sku.consignee and rd.sku = sku.sku " & _
                    "join (SELECT rd.CONSIGNEE, rd.ORDERID, sum(ISNULL(SKUUOM.VOLUME, 0) * qtyreceived) AS volume FROM receiptdetail AS rd INNER JOIN " & _
                    "SKUUOM ON rd.CONSIGNEE = SKUUOM.CONSIGNEE AND rd.SKU = SKUUOM.SKU WHERE (SKUUOM.LOWERUOM IS NULL) OR (SKUUOM.LOWERUOM = '') group by rd.CONSIGNEE, rd.ORDERID) AS rhvol ON rhvol.CONSIGNEE = rd.CONSIGNEE and  rhvol.ORDERID = rd.ORDERID " & _
                    ...
                    Made4Net.Shared.Util.FormatField(WMS.Lib.Actions.Audit.CREATELOAD), _
                    Made4Net.Shared.Util.FormatField(WMS.Lib.Statuses.Receipt.CLOSED), _
                    Made4Net.Shared.Util.FormatField(_agreement.Consignee), Made4Net.Shared.Util.FormatField(WMS.Lib.Actions.Audit.UNRECEIVELOAD), _
                    Made4Net.Shared.Util.FormatField(_storageclass))
            ...
            For Each dr In dt.Rows
                ...
                Select Case _billbasis
                    ...
                    Case AgreementDetail.BillingBasis.NumberOfDucments
                        ...
                        logger.Write("Transaction Type".PadRight(15) & "|" & "Document Id".PadRight(20) & "|" & "Billing base units".PadRight(20) & "|" & "Price per unit".PadRight(20) & "|" & "Calculated Value".PadRight(20) & "|")
                        logger.writeSeperator("-", 121)
                        TotalBillingValue = Me.getChargeValue(dt.Rows.Count, "INBOUND", dr("orderid"), 1, logger)
                        logger.Write("Number of document: " & dt.Rows.Count)
                        'Begin RWMS-809
                        logger.Write("Total Value: ".PadLeft(80) & TotalBillingValue)
                ...
                logger.Write("Transaction Type".PadRight(15) & "|" & "Document Id".PadRight(20) & "|" & "Billing base units".PadRight(20) & "|" & "Price per unit".PadRight(20) & "|" & "Calculated Value".PadRight(20) & "|")
                logger.writeSeperator("-", 121)
                TotalBillingValue = Me.getChargeValue(LineBilling, "INBOUND", dr("orderid"), 1, logger)
