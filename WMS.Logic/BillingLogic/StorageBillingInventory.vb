...
#Region "Constructors"
    ...
    Public Sub New(ByVal dr As DataRow)
        ...
        _uomweight = dr("grossweight")
        _uomvolume = dr("volume")
...
#Region "Methods"
    ...
    Public Function UpdateRunConditionForCollection(ByVal vRunCondtition As Int32, ByVal vStoragePeriodTime As Int32, ByVal StartPeriodDate As DateTime, ByVal LastPeriodDate As DateTime, Optional ByVal logger As WMS.Logic.LogHandler = Nothing)
...
Public Class StorageBillingProvider
        ...
        For Each dr As DataRow In dt.Rows
            ...
            Catch ex As Exception
                If Not logger Is Nothing Then
                    ...
                    logger.Write("Error adding storage when etracting current inventory: " & dr("CONSIGNEE") & dr("SKU"))
                    logger.writeSeperator()
                End If
            End Try
        Next
    ...
    Private Function GetRunConditionWhereClause(ByVal vRunCondtition As Int32, ByVal StartPeriodDate As DateTime, _
                                                ByVal LastPeriodDate As DateTime, ByVal oPeriodType As PeriodTypes, ByVal iPeriod As Int32)
    ...
    Public Function getStockForDate(ByVal StockDate As DateTime, ByRef ItemsCache As Hashtable, Optional ByVal logger As WMS.Logic.LogHandler = Nothing) As StorageBillingCollection
        ...
        For Each dr As DataRow In _trans.Rows
            ...
            oStorageBillingTransactionLine.LoadId = dr("LOADID")
            oStorageBillingTransactionLine.Consignee = _consignee
            oStorageBillingTransactionLine.Sku = dr("SKU")
            oStorageBillingTransactionLine.Units = dr("currentqty")
            oStorageBillingTransactionLine.TransactionDate = StockDate 'dr("TRANDATE") 'StockDate 'dr("units")
            Try
                Dim key As String = _consignee & "~" & dr("SKU")
                If ItemsCache.Contains(key) Then
                    oSku = ItemsCache(key)
                Else
                    oSku = New WMS.Logic.SKU(_consignee, dr("SKU"))
