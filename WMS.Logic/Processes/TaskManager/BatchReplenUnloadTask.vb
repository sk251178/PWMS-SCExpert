...
<CLSCompliant(False)> Public Class BatchReplenUnloadTask
    Inherits Task
...
#Region "Accessors"
    ...
    Public Function AssignTask(ByVal pUser As String)
        ...
        sql = String.Format("SELECT TOP 1 TASK FROM TASKS WHERE USERID = '{0}' AND ASSIGNED = 1 AND STATUS = '{1}' AND TASKTYPE = '{2}'", pUser, WMS.Lib.Statuses.Task.AVAILABLE, WMS.Lib.TASKTYPE.BRUNLOAD)
...
#Region "Create"
    ...
    Public Overloads Sub Create(ByVal batchReplenishment As String, ByVal puser As String)
        If _task = "" Then
            _task = Made4Net.Shared.Util.getNextCounter("TASK")
        End If
        _replenishment = batchReplenishment
        _fromwarehousearea = ""
        _priority = 200
        _task_type = WMS.Lib.TASKTYPE.BRUNLOAD
        MyBase.Post(puser)
    End Sub
