...
Partial Public Class NewQty2
    ...
    Private Sub doBack()
        Session.Remove("LoadStatusUpdateLoadId")
        Response.Redirect(MapVirtualPath("Screens/NewQty.aspx"))
    End Sub
    ...
    Private Sub doNext()
        ...
        Try
            ...
            If String.IsNullOrEmpty(DO1.Value("ReasonCode")) Then
                ...
                Return
            End If
            ManageMutliUOMUnits.AddUOMUnits(DO1.Value("UOM"), DO1.Value("NEWQTY"), "")
            ManageMutliUOMUnits.SetValUOMUnitsAfterDrow(DO1)
            If Not IsNumeric(ManageMutliUOMUnits.GetTotalEachUnits()) Then
                HandheldPopupNAlertMessageHandler.DisplayMessage(Me,  "NEWQTY field is mandatory.")
                Return
            End If
            'Dim oSku As New WMS.Logic.SKU(ld.CONSIGNEE, ld.SKU)
            ...
            'RWMS-1250
            If ld.UNITS = units Then
                HandheldPopupNAlertMessageHandler.DisplayMessage(Me, "Entered Quantity Is Same As Current Quantity.")
                DO1.Value("NEWQTY") = ""
                ManageMutliUOMUnits.Clear(True)
                doMenu()
            End If
            If Not String.IsNullOrEmpty(DO1.Value("UNITS")) Then
                If Convert.ToDecimal(DO1.Value("UNITS")) <> ld.UNITS Then
                    HandheldPopupNAlertMessageHandler.DisplayMessage(Me,  "Payload has been modified elsewhere.")
                    Return
                End If
            End If
            'RWMS-1250
            ld.Adjust(WMS.Lib.Actions.Audit.NEWQTY, units, DO1.Value("ReasonCode"), WMS.Logic.GetCurrentUser)
    ...
    Private Sub DO1_CreatedChildControls(ByVal sender As Object, ByVal e As System.EventArgs) Handles DO1.CreatedChildControls
        ...
        DO1.AddTextboxLine("NEWQTY", False, "new qty")
