...
<CLSCompliant(False)> Public Class PCKP
...
#End Region
    ...
    Private Sub MenuClick()
        ...
        If WMS.Logic.TaskManager.isAssigned(UserId, TASKTYPE.PARTIALPICKING, LogHandler.GetRDTLogger()) Then
            Dim tm As WLTaskManager = WLTaskManager.NewTaskManagerForUserAndTaskType(UserId, TASKTYPE.PARTIALPICKING, LogHandler.GetRDTLogger())
            tm.ExitTask()
        End If
    ...
    Private Sub NextClicked()
        ...
        If Not WLTaskManager.isAssigned(UserId, TASKTYPE.PARTIALPICKING, LogHandler.GetRDTLogger()) Then
            Try
                Dim tm As New WMS.Logic.TaskManager
                tm.RequestTask(UserId, WMS.Lib.TASKTYPE.PARTIALPICKING)
                CheckAssigned()
    ...
    Private Function CheckAssigned() As Boolean
        ...
        If Logic.TaskManager.isAssigned(UserId, WMS.Lib.TASKTYPE.PARTIALPICKING, rdtLogger) Then
            Dim pcklst As Picklist
            Try
                pcklst = New Picklist(tm.getAssignedTask(UserId, WMS.Lib.TASKTYPE.PARTIALPICKING, rdtLogger).Picklist)
            Catch ex As Exception
    ...
    Protected Sub setAssigned(ByVal pcklst As Picklist)
        ...
        Dim contid As String = pcklst.ActiveContainer()
        If contid = "" Then
            DO1.Value("ContainerId") = ""
        Else
        ...
        End If
        'DO1.Value("ContainerId") = pcklst.getNextContainerId()
