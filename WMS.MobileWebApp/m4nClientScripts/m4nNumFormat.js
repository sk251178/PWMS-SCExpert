/*
Made4Net Version: 2.1

This object provides formatting for numeric values
*/
...
function m4nNumFormat(val, decimals) {
	var num = new NumberFormat(val)
	num.setPlaces(decimals);
	return num.toFormatted()
}

/////////////////////////////////////////////////////////////
//wrapper object for NumberFormat function
/////////////////////////////////////////////////////////////
function m4nNum(val) {
	
	//public methods
	this.toFormatted		= numToFormatted;
	this.setDecimals		= numSetDecimals;
	this.setCommas			= numSetCommas;
	this.setSymbol			= numSetSymbol;
	this.setSymbolPosition	= numSetSymbolPosition;
	this.setNegativeFormat	= numSetNegativeFormat;
	this.setNegativeColor	= numSetNegativeColor;
	this.getNegativeColor	= numGetNegativeColor;
	
	//members
	this.negativeColor;
	
	//ctor
	this.nf = new NumberFormat(val)
	

	function numToFormatted() {
		return this.nf.toFormatted();
	}
	function numSetDecimals(val) {
		if (val==null) val = -1;
		if (val==undefined) val = -1;
		if (val<0) val = -1;
		
		this.nf.setPlaces(val);
	}
	function numSetCommas(val) {
		if (val==undefined) return;
		this.nf.setCommas(val);
	}
	function numSetSymbol(val) {
		if (val==undefined) return;
		this.nf.setCurrency(true);
		this.nf.setCurrencyPrefix(val);
	}
	function numSetSymbolPosition(val) {
		if (val==undefined) return;
		this.nf.setCurrencyPosition(val);
	}
	function numSetNegativeFormat(val) {
		if (val==undefined) return;
		this.nf.setNegativeFormat(val);
	}
	function numSetNegativeColor(val) {
		if (val==undefined) return;
		this.negativeColor = val;
	}
	function numGetNegativeColor(val) {
